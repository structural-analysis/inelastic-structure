...
def condense_boundary_condition(joints_restraints):
    # Computation of Natural Frequencies and Shape Modes
    jj = 0
    non_zero_joints_restraints = joints_restraints
    zero_mass_joints_restraints = joints_restraints
    jz = 0
    for bc_counter in range(len(non_zero_joints_restraints)):
        if (3*non_zero_joints_restraints[bc_counter-jj, 0]+non_zero_joints_restraints[bc_counter-jj, 1]) % 3 == 2:
            non_zero_joints_restraints = np.delete(non_zero_joints_restraints, bc_counter-jj, 0)
            jj += 1
        elif (3*non_zero_joints_restraints[bc_counter-jj, 0]+non_zero_joints_restraints[bc_counter-jj, 1]) % 3 != 2:
            non_zero_joints_restraints[bc_counter-jj, 1] = non_zero_joints_restraints[bc_counter-jj, 1]-(3*non_zero_joints_restraints[bc_counter-jj, 0]+non_zero_joints_restraints[bc_counter-jj, 1])//3

        if (3*zero_mass_joints_restraints[bc_counter-jz, 0]+zero_mass_joints_restraints[bc_counter-jz, 1]) % 3 == 2:
            zero_mass_joints_restraints[bc_counter-jz, 1] = zero_mass_joints_restraints[bc_counter-jz, 1]-2*((3*zero_mass_joints_restraints[bc_counter-jz, 0]+zero_mass_joints_restraints[bc_counter-jz, 1])//3+1)
        elif (3*zero_mass_joints_restraints[bc_counter-jz, 0]+zero_mass_joints_restraints[bc_counter-jz, 1]) % 3 != 2:
            zero_mass_joints_restraints = np.delete(zero_mass_joints_restraints, bc_counter-jz, 0)
            jz += 1
    return non_zero_joints_restraints, zero_mass_joints_restraints


def static_condensation(structure_stiffness, structure_mass, joints_restraints):
    non_zero_joints_restraints, zero_mass_joints_restraints = condense_boundary_condition(joints_restraints)
    mtt = structure_mass
    ...
    for i in range(n):
        ...
        k0t = np.delete(k0t, 3*i+1-jj, 0)
        jj += 1
    kttr = apply_boundry_conditions(non_zero_joints_restraints, ktt)
    mttr = apply_boundry_conditions(non_zero_joints_restraints, mtt)
    k00r = apply_boundry_conditions(zero_mass_joints_restraints, k00)
    k0tr = apply_boundry_conditions(joints_restraints, k0t)
...
def load_condensation(force, ku0, joints_restraints):
    ...
    for i in range(n):
        pt = np.delete(pt, 3*i+2-ii, 0)  # delete row
        ii += 1
        p0 = np.delete(p0, 3*i+1-jj, 0)  # delete row
        jj += 1
        p0 = np.delete(p0, 3*i+1-jj, 0)  # delete row
        jj += 1
    ptr = apply_boundry_conditions(non_zero_joints_restraints, pt)

    p0r = apply_boundry_conditions(zero_mass_joints_restraints, p0)
