    ...
    if structure.is_inelastic:
        ...
        for time_step in range(1, initial_analysis.time_steps):
            ...
            for i in range(increments_count):
                ...
                members_nodal_strains = elastic_members_nodal_strains_history[time_step, :] * load_limit
                members_nodal_stresses = elastic_members_nodal_stresses_history[time_step, :] * load_limit

                members_nodal_forces[i, :, :] = elastic_members_nodal_forces
                members_disps[i, :, :] = elastic_members_disps

                if has_any_response(members_nodal_strains):
                    nodal_strains[i, :] = average_nodal_responses(structure=structure, members_responses=members_nodal_strains)
                    nodal_stresses[i, :] = average_nodal_responses(structure=structure, members_responses=members_nodal_stresses)
